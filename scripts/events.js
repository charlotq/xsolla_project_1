...
const eventTemplate = document.querySelector("#events__item").content;
let day;
const citySelect = document.querySelector("select[name = city]");
const monthSelect = document.querySelector("select[name = month]");

const setEvent = (title, link, day) => {
  const event = eventTemplate.cloneNode(true);
  const eventTitle = event.querySelector(".events__title");
  const eventImage = event.querySelector(".events__image");
  const eventDay = event.querySelector(".events__day-text");
...
const filterMonth = (month, array) => {
  const filterArray = array.filter(item => {
    return item.date[3] + item.date[4] === month;
  });
  console.log(filterArray);
  return filterArray;
};

const filterAll = (city, month, array) => {
  const filterArray = array.filter(item => {
    return (item.city === city) && (item.date[3] + item.date[4] === month);
  });
  console.log(filterArray);
  return filterArray;
};
...
const setEvents = (array) => {
  array.forEach((elem) => {
    day = elem.date[0] + elem.date[1];
    events.append(setEvent(elem.name, elem.image, day));
  });
}
...
const checkSelects = () => {
  if ((citySelect.value !== "none") && (monthSelect.value !== "none")) setEvents(filterAll(citySelect.value, monthSelect.value, eventsArray));
  if ((citySelect.value === "none") && (monthSelect.value !== "none")) setEvents(filterMonth(monthSelect.value, eventsArray));
  if ((citySelect.value !== "none") && (monthSelect.value === "none")) setEvents(filterCity(citySelect.value, eventsArray));
  if ((citySelect.value === "none") && (monthSelect.value === "none")) setEvents(eventsArray);
};
